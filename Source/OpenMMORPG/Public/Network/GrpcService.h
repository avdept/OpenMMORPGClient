...
template <typename  ReqStub, typename Request, typename  Response, typename Lambda>
grpc::Status SendServerRequest(Request &Req, Response &Resp, Lambda &lamb, FString Token, FString CharacterID)
{

    std::unique_ptr<ReqStub::Stub> ServiceStub = ReqStub::NewStub(USocketObject::GRPCChannel);

    grpc::ClientContext context;
    FString Bearer = FString::Printf(TEXT("Bearer %s"), *Token);
    std::string const BearerStr = std::string(TCHAR_TO_UTF8(*Bearer));
